	...
	t.Run("all_sources_enabled", func(t *testing.T) {
		...
		cfg := &Config{
			Sources: map[string]PluginConfig{
				...
				SourceWolframAlpha:       {Enabled: true}, // v6 cycle 5 / v2.18.0
				SourceKGAPI:              {Enabled: true}, // v6 cycle 5 / v2.18.0
				SourceNewsAPI:            {Enabled: true}, // v6 cycle 6 / v2.19.0
				SourceSerpAPINews:        {Enabled: true}, // v6 cycle 6 / v2.19.0
			},
		}
		plugins, err := InitializePlugins(cfg, logger)
		require.NoError(t, err)
