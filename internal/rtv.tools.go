...
NOT EVERY SOURCE CAN BE FETCHED BY ID. Only 21 of the 61 plugins have a per-record retrieval API — essentially the id-addressable scholarly, package and encyclopedia sources. Every web and news source (linkup, brave, exa, perplexity, firecrawl, kagi, mojeek, serpapi, newsapi, gdelt, ...) is search-only: its result id is a one-way hash of the page URL and rtv_get against it is refused immediately with "source does not support get-by-id". Check supports_get in rtv_list_sources BEFORE calling. When supports_get is false, the search result's "url" field (always present) is THE way to reach the content — hand it to a page-fetching tool; retrievr does not fetch pages.`

	ToolDescListSources = `List every registered source (61 plugins) with full capability surface: content_types served, native + available formats, residency posture (region + DPA status), supported result kinds (paper/web/code/place/...), query intents, rate limits, sort/filter capabilities, max_results_per_query, requires_credential, free_tier flag, and supports_get. supports_get=false means rtv_get can NEVER succeed for that source (all web + news providers) — use the search result's "url" instead. Use this to pick sources by intent + jurisdiction + filter support without trial-and-error.`
)
